  ...
  <div>
    ...
    <div class="row space-top">
      <div class="col-md-6">
        <h4>Items</h4>
        <hr />
        <div class="row" v-if="form.reading_list.items.length > 0">
          <div class="col-md-12">
            <div class="panel-group" role="tablist">
              <div class="panel panel-default"  v-for="item in form.reading_list.items">
  ...
  methods: {
    remove(item) {
      ...
      if (ack) {
        axios
          .delete(
            `/api/escapes/${this.form.id}/reading_list/${this.form.reading_list
              .id}/items/${item.id}`
          )
          .then(response => {
            this.form.reading_list.items = response.data;
            this.clear();
          });
      }
    },
    save() {
      let resource = `/api/escapes/${this.form.id}/reading_list/${this.form
      ...
      if (this.itemForm.id) {
        Form.put(
          ...
          this.form.reading_list.items = response;
          this.$emit('save');
          this.clear();
        });
      } else {
